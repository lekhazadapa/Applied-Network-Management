...
#"CDEF:aggOut$id$n=outBytes$id$n,+",
									"LINE1:inBytes$id$in$hexa:inbytes-$ip-$in","COMMENT:\\n","GPRINT:inBytes$id$in:MAX:max %6.2lf %SBps","GPRINT:inBytes$id$in:AVERAGE:avg %6.2lf %SBps","GPRINT:inBytes$id$in:LAST:last %6.2lf %SBps\\n","LINE2:outBytes$id$in$hexa1:Outbytes-$ip-$in","COMMENT:\\n","GPRINT:outBytes$id$in:MAX:max %6.2lf %SBps ","GPRINT:outBytes$id$in:AVERAGE:avg %6.2lf %SBps","GPRINT:outBytes$id$in:LAST:last %6.2lf %SBps\\n");#"GPRINT:addIn$id$in:LAST: %6.2lf %SBps\\n","GPRINT:aggOut$id$in:LAST: %6.2lf %SBps\\n"#);
  ...
  if (! $ret) {
    echo "<b>Graph error: </b>"."\n".rrd_error()."\n";
				...
				echo "<center><img src='device.png' height='4000' width='1100' alt='Generated RRD image' ></center>";
