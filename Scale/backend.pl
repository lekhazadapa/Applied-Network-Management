...
my $ddl="CREATE TABLE IF NOT EXISTS `sassign4` (`id` int(11) NOT NULL AUTO_INCREMENT, 
...
my $sth1=$dbh->prepare("SELECT * FROM sassign4 where IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth1->execute();
...
{
#print"not existed\n";
$sth2=$dbh->prepare("INSERT INTO `sassign4` (`IP`,`PORT`,`COMMUNITY`,`UPTIME`, `SUCCESS`,`FAIL1`, `TOTAL_REQ`, `TIME`) VALUES ('$IP', '$PORT', '$COMMUNITY', '$sysuptime', '1', 0,'1', '$time')");
$sth2->execute();
}
else
{#print "existed\n"; 
$sth3=$dbh->prepare("SELECT `SUCCESS`, `TOTAL_REQ` FROM `sassign4` WHERE IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth3->execute();
...
$sth4=$dbh->prepare("UPDATE `sassign4` SET SUCCESS=$SUCCESS, TOTAL_REQ=$TOTAL_REQ, FAIL1='0' WHERE IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth4->execute();
}
}
...
{
 $sth5=$dbh->prepare("SELECT * FROM sassign4 where IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth5->execute();
...
$sth6=$dbh->prepare("INSERT INTO `sassign4` (`IP`,`PORT`,`COMMUNITY`,`UPTIME`, `FAIL`,`FAIL1`,`TOTAL_REQ`, `TIME`) VALUES ('$IP', '$PORT', '$COMMUNITY', 'NOT_DEFINED', '1','1', '1', '$time')");
$sth6->execute();
}
else
{#print " '$IP', '$PORT', '$COMMUNITY'  existed\n"; 
$sth7=$dbh->prepare("SELECT `FAIL`, `TOTAL_REQ` FROM `sassign4` WHERE IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth7->execute();
...
$sth8=$dbh->prepare("UPDATE `sassign4` SET FAIL=$FAIL, FAIL1=$FAIL,TOTAL_REQ=$TOTAL_REQ1 WHERE IP='$IP' AND PORT='$PORT' AND COMMUNITY='$COMMUNITY' ");
$sth8->execute();
}
}
}
